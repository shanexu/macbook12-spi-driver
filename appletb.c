			    ...
			    struct hid_usage *usage, __s32 value)
{
	...
	if (tb_data->cur_tb_mode == APPLETB_CMD_MODE_OFF ||
	    tb_data->cur_tb_disp == APPLETB_CMD_DISP_OFF) {
		input_event(field->hidinput->input, EV_KEY, KEY_LEFTSHIFT, 1);
		input_event(field->hidinput->input, EV_KEY, KEY_LEFTSHIFT, 0);
		rc = 1;
 ...
 * to hid-core's hid_ignore_list. This means the hid-generic will hid driver
